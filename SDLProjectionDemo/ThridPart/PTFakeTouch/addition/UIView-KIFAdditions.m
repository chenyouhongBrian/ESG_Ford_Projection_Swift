...
- (void)tapAtPoint:(CGPoint)point;
{
    ...
    if (webBrowserView) {
        NSString *selectorEncoded = @"dGFwSW50ZXJhY3Rpb25XaXRoTG9jYXRpb246";     // tapInteractionWithLocation:
        SEL selector = NSSelectorFromString([selectorEncoded stringBase64Decoded]);
        
        NSMethodSignature *signature = [self methodSignatureForSelector:selector];
        NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];
        [invocation setTarget:webBrowserView];
...
- (void)dragPointsAlongPaths:(NSArray *)arrayOfPaths {
    ...
    NSUInteger pointsInPath = [(NSArray *)arrayOfPaths[0] count];
